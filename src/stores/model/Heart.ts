...
const Heart = types
  ...
  .actions(self => {
    const fetchHeart = flow(function* () {
      const response: RetrieveAsyncFunc<typeof checkMyHeartUsingGET> =
        yield checkMyHeartUsingGET();
    ...
    const useHeart = flow(function* () {
      ...
      const response: RetrieveAsyncFunc<typeof useHeartUsingPUT> =
        yield useHeartUsingPUT();
      self.heartCount = response.heartCount ?? 0;
      self.leftTime = today().add(response.leftTime ?? 0, 'seconds');
    });
