...
const SingerStore = types
  ...
  .actions(self => {
    const initializeSingers = flow(function* () {
      const responseSingers: RetrieveAsyncFunc<typeof singers> =
        yield singers();
    ...
    const initializeRegisteredSingers = flow(function* () {
      const responseRegisteredSingers: RetrieveAsyncFunc<
        typeof registeredSingers
      > = yield registeredSingers();
