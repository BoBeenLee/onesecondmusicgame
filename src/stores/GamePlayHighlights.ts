...
const GamePlayHighlights = types
  ...
  .actions(self => {
    const initialize = flow(function* (selectedSingers: ISinger[]) {
      const response: RetrieveAsyncFunc<typeof getHighlightListUsingPOST> =
        yield getHighlightListUsingPOST({
    ...
    const isAnswer = flow(function* (userAnswer: string) {
      const response: RetrieveAsyncFunc<typeof isAnswerUsingPOST> =
        yield isAnswerUsingPOST({
          answer: userAnswer,
