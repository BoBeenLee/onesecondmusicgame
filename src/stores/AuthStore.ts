...
const AuthStore = types
  ...
  .actions(self => {
    ...
    const initialize = flow(function* () {
      const {provider, accessId, accessToken, refreshToken} =
    ...
    const kakaoSignIn = flow(function* () {
      try {
        const tokenResponse: RetrieveAsyncFunc<typeof KakaoLogin.login> =
          yield KakaoLogin.login();

        if ('accessToken' in tokenResponse) {
          self.accessId = tokenResponse.idToken;
    ...
    const googleSignIn = flow(function* () {
      try {
        const userInfo: RetrieveAsyncFunc<typeof GoogleSignin.signIn> =
          yield GoogleSignin.signIn();
        const tokenInfo: RetrieveAsyncFunc<typeof GoogleSignin.getTokens> =
          yield GoogleSignin.getTokens();
    ...
    //   const result: RetrieveAsyncFunc<
    //     typeof LoginManager.logInWithPermissions
    //   > = yield LoginManager.logInWithPermissions(['public_profile']);
    //   if (result.isCancelled) {
    //     // throw new Error("Login cancelled");
    //     return false;
    //   }
    //   const data: RetrieveAsyncFunc<typeof AccessToken.getCurrentAccessToken> =
    ...
    const appleSignIn = flow(function* () {
      const appleAuthRequestResponse: RetrieveAsyncFunc<
        typeof appleAuth.performRequest
      > = yield appleAuth.performRequest({
        requestedOperation: AppleRequestOperation.LOGIN,
        requestedScopes: [AppleRequestScope.EMAIL, AppleRequestScope.FULL_NAME],
      });

      const {user, identityToken /* etc */} = appleAuthRequestResponse;
      const credentialStateResponse: RetrieveAsyncFunc<
        typeof appleAuth.getCredentialStateForUser
      > = yield appleAuth.getCredentialStateForUser(user);

      if (!identityToken) {
        throw new Error('not exists identityToken');
      }
      if (credentialStateResponse !== AppleCredentialState.AUTHORIZED) {
    ...
    const signIn = flow(function* () {
      const signInResponse: RetrieveAsyncFunc<typeof signInUsingPOST> =
        yield signInUsingPOST({
          accessId: self.accessId,
    ...
    const signUp = flow(function* ({nickname}: {nickname: string}) {
      const deviceId: RetrieveAsyncFunc<typeof getUniqueID> =
        yield getUniqueID();
    ...
    const updateUserReward = flow(function* () {
      const response: RetrieveAsyncFunc<typeof findItemAllUsingGET> =
        yield findItemAllUsingGET();
